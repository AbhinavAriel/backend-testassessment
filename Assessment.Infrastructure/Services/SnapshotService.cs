...
namespace Assessment.Infrastructure.Services
{
    public class SnapshotService : ISnapshotService
    {
        ...
        public SnapshotService(ApplicationDbContext db, HttpClient http, IConfiguration config)
        {
            ...
            _projectUrl = config["Supabase:ProjectUrl"]
                ?? throw new InvalidOperationException("Supabase:ProjectUrl is missing.");
        ...
        public async Task<SnapshotResponseDto> UploadAsync(UploadSnapshotRequestDto dto)
        {
            ...
            try
            {
                var base64 = dto.ImageData.Contains(',')
                    ? dto.ImageData.Split(',')[1]
                    : dto.ImageData;
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    throw new InvalidOperationException($"Supabase upload failed: {body}");
                }
        ...
        public async Task<List<SnapshotResponseDto>> GetByTestIdAsync(Guid testId)
        {
            ...
            if (!snapshots.Any())
                throw new KeyNotFoundException("No snapshots found.");
        ...
        public async Task<(byte[] Bytes, string ContentType)> GetImageAsync(Guid snapshotId)
        {
            if (snapshotId == Guid.Empty)
                throw new ArgumentException("Invalid snapshotId.");
            ...
            if (!res.IsSuccessStatusCode)
                throw new InvalidOperationException("Failed to fetch image from storage.");
